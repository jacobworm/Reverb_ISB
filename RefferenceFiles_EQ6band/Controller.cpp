...
float Controller::incParamValue()
{
	...
	m_eqLeft->incParameter(m_band, m_param, 0.1);
	float val = m_eqRight->incParameter(m_band, m_param, 0.1);
	return val;
}
...
float Controller::decParamValue()
{
	...
	m_eqLeft->decParameter(m_band, m_param, 0.1);
	float val = m_eqRight->decParameter(m_band, m_param, 0.1);
	return val;
}

PARAMETER Controller::getParam(){
	return m_param;
}
...
void Controller::printParam(void) 
{
	char colors[NUM_EQ_BANDS] = { 'R', 'G', 'B', 'W'};
	for (int band = 0; band < NUM_EQ_BANDS; band++) {
		float val, freq;
		val = m_eqLeft->getParameter(band, PM_GAIN);
		freq = m_eqLeft->getParameter(band, PM_FREQ);
		m_daisyPod->seed.PrintLine("#BAND %c%d Freq %5.0f Gain %.1f", colors[band], band+1, freq, val);
	}
}