...
float Equalizer::Process(float input)
{
	...
	if (filterOn_) {
		for (int i = 0; i < NUM_EQ_BANDS; i++){
			output = m_IIRfilter[i].Process(output);
		}
	}		
	return output;
}
...
void Equalizer::Init(int sampleRate)
{
	...
	m_EQParams[0].gain = 0.0;
	m_EQParams[1].fc = 400.0;
	m_EQParams[1].gain = 0.0; 
	m_EQParams[2].fc = 1000.0;
	m_EQParams[2].gain = 0.0; 
	m_EQParams[3].fc = 2000.0;
	m_EQParams[3].gain = 0.0;
	m_EQParams[4].fc = 4000.0;
	m_EQParams[4].gain = 0.0;
	m_EQParams[5].fc = 8000.0;
	m_EQParams[5].gain = 0.0; 
	...
	for (short band = 0; band < NUM_EQ_BANDS; band++)
	{
		...
		m_EQParams[band].Q = 2.0; // Default Q
...
float Equalizer::incParameter(short band, PARAMETER param, float delta)
{
	...
	// Frekvensparameter vokser med en ratio. Skal evt være delta multipliceret med en konstant (her 12. rod af 2 for at få 12 trin per oktav)
	if(param == PM_FREQ){
		setParameter(band, param, m_EQParams[band].fc * pow(2, 1.0/12.0));
		updateEQParameters(band); 
...
{
    bool update = false;
	// float value_dB = pow(10.0,(value/20.0)); // Converting to dB
...
void Equalizer::updateEQParameters(short band)
{
	...
	if (band == 0){
		m_IIRfilter[band].makeLowShelvingEQ(sampleRate_,
							      m_EQParams[band].fc,
								  m_EQParams[band].Q,
								  m_EQParams[band].gain);
	}
	else if (band == 5){
		m_IIRfilter[band].makeHighShelvingEQ(sampleRate_,
							      m_EQParams[band].fc,
								  m_EQParams[band].Q,
								  m_EQParams[band].gain);
	}
